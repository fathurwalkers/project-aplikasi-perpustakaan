            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    @foreach ($kategori as $item2)
                        <button class="dropdown-item btn btn-primary buttonkategori"
                            value="{{ $item2->id }}">{{ $item2->kategori_nama }}</button>
                    @endforeach
                </div>
            </div>
        </div>
            ...
            <div class="col-sm-6 col-md-6 col-lg-6 mb-4">
                <div class="card h-100 w-100" style="s">
                    <div class="card-body">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted text-dark text-center bukujudul">{{ $item->buku_judul }}
                        </h6>
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td class="fontubah bukutotalpinjam">&nbsp;: {{ $item->buku_support_rekomendasi }} Kali di
                                    pinjam
                                    @switch($item->buku_support_rekomendasi)
                                        @case($item->buku_support_rekomendasi <= 19)
                                            {{-- <span class="badge badge-primary py-auto" id="">
                                                Ordinary
                                            </span> --}}
                                        @break

                                        @case($item->buku_support_rekomendasi >= 20 && $item->buku_support_rekomendasi <= 49)
                                            <span class="badge badge-secondary py-auto" id="">
                                                Rekomendasi
                                            </span>
                                        @break

                                        @case($item->buku_support_rekomendasi >= 50 && $item->buku_support_rekomendasi <= 99)
                                            <span class="badge badge-warning py-auto" id="">
                                                Populer
                                            </span>
                                        @break

                                        @case($item->buku_support_rekomendasi >= 100)
                    ...
                    <button class="btn btn-md rounded btn-success mt-auto mx-4 mb-3 counters bukuid clickbutton"
                        onclick="storKeranjang({{ $item->id }})" value="{{ $item->id }}">Tambah Pinjaman</button>
                </div>
            </div>
        @endforeach
                ...
                success: function(data) {
                    ...
                    if (buku_ukuran == 0) {
                        ...
                        for (let i = 0; i < buku_ukuran; i++) {
                            ...
                            output +=
                                '<h6 class="card-subtitle mb-2 text-muted text-dark text-center bukujudul">' +
                                buku_get['buku_judul'] + '</h6>';
                            output += '<table>';
                            output += '     <tr>';
                            output += '     <td class="fontubah">Penulis </td>';
                            output += '                  <td class="fontubah bukupenulis">&nbsp; : ' +
                                buku_get['buku_penulis'] + '</td>';
                            output += '        </tr>';
                            output += '        <tr>';
                            output += '            <td class="fontubah">Penerbit </td>';
                            output += '            <td class="fontubah bukupenerbit">&nbsp;: ' +
                                buku_get['buku_penerbit'] + '</td>';
                            output += '        </tr>';
                            output += '            <tr>';
                            output += '                <td class="fontubah">Tahun Terbit </td>';
                            output += '                <td class="fontubah bukutahunterbit">&nbsp;: ' +
                                buku_get['buku_tahunterbit'] + '</td>';
                            output += '            </tr>';
                            output += '            <tr>';
                            output += '                <td class="fontubah">Jumlah Halaman </td>';
                            output +=
                                '                <td class="fontubah bukujumlahhalaman">&nbsp;: ' +
                                buku_get['buku_jumlahhalaman'] + ' Halaman</td>';
                            output += '            </tr>';
                            output += '            <tr>';
                            output += '                <td class="fontubah">Total Peminjam </td>';
                            output += '<td class="fontubah bukutotalpinjam">&nbsp;: ' + buku_get[
                                'buku_support_rekomendasi'] + ' Kali di pinjam';
                            ...
                            if (buku_get['buku_support_rekomendasi'] >= 11 && buku_get[
                                    'buku_support_rekomendasi'] <= 20) {
                                output += '<span class="badge badge-secondary py-auto" id="">';
                                output += ' Populer';
                                output += '</span>';
                            }

                            if (buku_get['buku_support_rekomendasi'] >= 21 && buku_get[
                                    'buku_support_rekomendasi'] <= 40) {
                                output += '<span class="badge badge-warning py-auto" id="">';
                                output += ' Terpopuler';
                                output += '</span>';
                            }

                            if (buku_get['buku_support_rekomendasi'] >= 41 && buku_get[
                                    'buku_support_rekomendasi'] <= 60) {
                                output += '<span class="badge badge-success py-auto" id="">';
                                output += ' Paling Populer';
                                output += '</span>';
                            }

                            if (buku_get['buku_support_rekomendasi'] >= 61 && buku_get[
                                    'buku_support_rekomendasi'] <= 100) {
                            ...
                            output += '     <td class="fontubah">&nbsp;: ' + buku_get[
                                'buku_kodekategori'] + '</td>';
                            output += ' </tr>';
                            // output += '  <tr>';
                            ...
                            output += '</div>';
                            output +=
                                '<button class="btn btn-md rounded btn-success mt-auto mx-4 mb-3 counters bukuid clickbutton" onclick="storKeranjang(' +
                                buku_get['id'] + ')" value="' + buku_get['id'] +
